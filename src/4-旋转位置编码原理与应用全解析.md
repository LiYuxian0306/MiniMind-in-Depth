...
**其实是不是主要不是这个问题** 更多的是计算效率问题？还是说从效果上发现确实区别很大？

# 正余弦位置编码的问题
思路来源于(sin pe问题讨论)[https://cloud.tencent.com/developer/article/2403895]   

[kv cache](https://zhuanlan.zhihu.com/p/662498827)是transformer decoder inference时候常用的技术

## 位置编码深度解析：Sinusoidal PE vs. RoPE

本文档详细推导了正弦位置编码 (Sinusoidal PE) 与旋转位置编码 (RoPE) 在数学原理上的本质区别，重点分析它们如何将位置信息注入模型以及在 Attention 计算中的表现。
